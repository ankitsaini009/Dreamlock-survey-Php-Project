...
function getRedirectLink($status, $supplier_id) {
    ...
    if (!$supplier) {
        error_log("Supplier not found for supplier_id: " . $supplier_id);
        return null;
    }
    ...
    $redirect_links = [
        ...
        'QT' => $supplier['quality_term_url'],
        'SC' => $supplier['survey_close_url'],
        'Q' => $supplier['over_quota_url'],
        'quality_term' => $supplier['quality_term_url'],
    ];

    return $redirect_links[$status] ?? null;
}
...
function getMessage($status) {
    $messages = [
        ...
        'quality_term' => 'Thanks for participating! Your response has been recorded for quality purposes.',
    ];
...
if (isset($_GET['hash_identifier']) && isset($_GET['status'])) {
    ...
    if ($surveyLog) {
        ...
        // Get the redirection link based on the status and supplier id
        $redirect_url = getRedirectLink($status, $surveyLog['Supplier_Id']);
        $message = getMessage($status);

        if (!$redirect_url) {
            error_log("No redirect URL found for status: " . $status . " and supplier_id: " . $surveyLog['Supplier_Id']);
        }
    } else {
        $redirect_url = null;
